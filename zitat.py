...
def open_clippings_file( filename ):
    ...
    try:
        f = open( filename )
        file_as_string = unicode( f.read(), 'utf-8-sig' )
        f.close()
...
def process_clipping( authors, c ):
    '''
    ...
    author, title, cltype, location, date, content = get_data( c )
    ...
    determiners = ( 'the ', 'a ', 'an ', 'o ', 'os ', 'as ', 'le ', 'les ', 'la ', 'der ', 'die ', 'das ' )
    if title.lower().startswith( determiners ):
        p = title.find( ' ' )
        title = title[p + 1:] + ', ' + title[:p] 
...
def import_clippings_file( filename ):
    ...
    clip_separator = '\r\n==========\r\n'
    clip_list = clippings_string.split( clip_separator )[:-1]
    
    # Initialize dictionary and process each clipping.
    authors = {}
    for c in clip_list:
        process_clipping( authors, c )
...
def export_author_org( a, f ):
    s = u'* {}\n\n'.format( a )
    f.write( s )


def export_title_org( t, f ):
    s = u'*** {}\n\n'.format( t )
    f.write( s ) 


def export_type_org( typ, f ):
    s = u'***** {}s\n\n'.format( typ )
    f.write( s )
...
def snippet( s, n ):
    '''
    Return prefix of s that is at most n chars long. 
    :param s: string.
    :param n: length of snippet.
    '''
...
def export_content_org( c, f ):
    ...
    body = format( fill( content, width=fill_col ) ) + '\n\n'
    f.write( body )
...
def export_to_org( clippings, out_filename, in_filename ):
    ...
    try:
        ...
        f.write( u'\n\n' )
        f.write( u'Generated from clippings in file ' + in_filename + '\n' )
        f.write( u'on ' + time.strftime( '%Y-%m-%d %H:%M:%S %Z' ) + '\n' )
        f.write( u'by the zitat script (http://sagallesab.wordpress.com/zitat).' )
        f.write( u'\n\n\n' )
        ...
        sorted_authors = sortedDict( clippings )
        for a, author_entry in sorted_authors:
            export_author_org( a, f )
            sorted_titles = sortedDict( author_entry )
            for t, title_entry in sorted_titles:
                export_title_org( t, f )
                sorted_types = sortedDict( title_entry )
                for typ, type_entry in sorted_types:
                    export_type_org( typ, f )
                    type_entry = sort_clipping_list( type_entry )
                    for c in type_entry:
                        export_content_org( c, f )
        f.close()
    except IOError:
...
def zitat( argv ):
    ...
    if len( argv ) > 2:
        ...
        output_filename = raw_input( 'Enter output filename below [{}]:\n>>> '.format( output_filename_sugg ) ).strip()
        if output_filename == '':
